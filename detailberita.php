...
<div class="box box-danger">
            <div class="box-header with-border">
              <h7 class="box-title"><a href="?mod=info_berita"> <i class="fa fa-angle-left"> Back</i> </a></h7>            
            </div>      
            <!-- /.box-header -->
            <div class="box-body">
            <?php
session_start();
  ...
  include "config/fungsi_kalender.php";
   include "config/fungsi_seo.php";
           $detail=mysqli_query($mysqli,"SELECT * FROM tbl_rpt_bencana WHERE id = '".abs((int)$_GET[id])."'");
  $d   = mysqli_fetch_array($detail);
  $uid=$d['uid'];
   $detail2=mysqli_query($mysqli,"select * FROM tbl_rpt_bencana, user WHERE tbl_rpt_bencana.uid=user.uid AND tbl_rpt_bencana.uid = '".$uid."'");
  $d2   = mysqli_fetch_array($detail2);
 $nama=$d2['nama'];
 ...
 // echo "<b>$d[keterangan]</b><br />";
  ...
  if ($jml > 0){
    ...
    $jmldata     = mysql_num_rows(mysql_query("SELECT * FROM tbl_komentar WHERE tbl_id_berita='".$val->validasi($_GET['id'],'sql')."' AND aktif='Y'"));
